...
def binomial_explanation(figsize=(9, 7.5)):
    ...
    def draw_row(ax, y, labels, colors, r=0.32, gap=0.85, label_color="white",
                fontsize=13, edge_colors=None):
        """Draw a row of N circles at height y, each with a number label."""
        x0 = 0.5
        for i in range(N):
            cx = x0 + i * gap
            ec = edge_colors[i] if edge_colors is not None else EDGE
            circ = patches.Circle((cx, y), r, facecolor=colors[i],
                                edgecolor=ec, linewidth=1.6, zorder=2)
            ax.add_patch(circ)
            if labels is not None:
                ax.text(cx, y, str(labels[i]), ha="center", va="center",
                        fontsize=fontsize, fontweight="bold",
                        color=label_color[i] if isinstance(label_color, list) else label_color,
                        zorder=3)
        return x0, gap
    ...
    y_a = 9.3
    draw_row(ax, y_a, labels=list(range(N)), colors=DISTINCT_COLORS,
            label_color="white", gap=gap)
    ax.text(4.75, y_a + 0.65, r"10 distinguishable balls $\rightarrow$ 10! orderings",
            ha="center", va="center", fontsize=13)

    # ---- Arrow down ----
    ax.annotate("", xy=(4.75, 8.15), xytext=(4.75, 8.7),
                arrowprops=dict(arrowstyle="-|>", color="#555555", lw=2))
    ax.text(5.6, 8.42, "fix which 3\nare \"black\"", fontsize=10.5, color="#444444",
            ha="left", va="center")
    ...
    for row_i, y in enumerate(y_positions):
        ...
        for pos in range(N):
            if pos in BLACK_POS:
                labels.append(bl_row[bi]); bi += 1
                colors.append(BLACK); label_colors.append("white"); edge_colors.append(EDGE)
            else:
                labels.append(wl_row[wi]); wi += 1
                colors.append(WHITE); label_colors.append("#1a1a1a"); edge_colors.append(EDGE)
        draw_row(ax, y, labels=labels, colors=colors, label_color=label_colors,
                gap=gap, edge_colors=edge_colors)

    ax.text(9.2, (7.3 + 5.4) / 2, r"$3! \times 7!$" + "\nlabelings,\nsame pattern",
            ha="left", va="center", fontsize=11.5, color="#444444")
    # brace-like bracket on the right of the three rows
    brace_x = 8.85
    ax.plot([brace_x, brace_x + 0.1, brace_x + 0.1, brace_x],
            [5.05, 5.05, 7.6, 7.6], color="#888888", lw=1.3)

    ax.text(4.75, 4.75, "...", ha="center", va="center", fontsize=18, color="#888888")

    # ---- Arrow down to final color-only pattern ----
    ax.annotate("", xy=(4.75, 3.55), xytext=(4.75, 4.35),
                arrowprops=dict(arrowstyle="-|>", color="#555555", lw=2))
    ax.text(5.6, 3.95, "discard labels,\nkeep only color", fontsize=10.5,
...
def analysis_path(figsize=(3.0, 7.2)):
    ...
    STEP6 = "#e0632b"   # enrichment
    STEP7 = "#4d9078"   # GO output
    TEXT_LIGHT = "#ffffff"
    CONNECTOR = "#8a97a8"
    ...
    boxes = [
        ...
        dict(label="Enrichment\nanalysis", color=STEP6),
        dict(label="GO terms", color=STEP7),
    ]
    ...
    for i, b in enumerate(boxes):
        ...
        shadow = FancyBboxPatch(
            (cx - box_w / 2 + 0.05, y - h / 2 - 0.05), box_w, h,
            boxstyle="round,pad=0.04,rounding_size=0.14",
            facecolor="#000000", edgecolor="none", alpha=0.15, zorder=1,
        )
        ax.add_patch(shadow)

        box = FancyBboxPatch(
            (cx - box_w / 2, y - h / 2), box_w, h,
            boxstyle="round,pad=0.04,rounding_size=0.14",
            facecolor=b["color"], edgecolor="white", linewidth=1.6, zorder=2,
        )
        ax.add_patch(box)

        ax.text(cx, y, b["label"], ha="center", va="center",
                 fontsize=11.5, fontweight="bold", color=TEXT_LIGHT,
                 zorder=3, linespacing=1.3)

        if i > 0:
            prev_y = positions[i - 1]
            ax.annotate(
                "", xy=(cx, y + h / 2), xytext=(cx, prev_y - h / 2),
                arrowprops=dict(arrowstyle="-|>", color=CONNECTOR, lw=2.2,
                                 shrinkA=0, shrinkB=0),
                zorder=1,
            )
...
def hyperg_vs_binom(figsize=(8, 7)):
    ...
    def draw_coin(ax, cx, cy, r, face=None, faded=False):
        ...
        circle = patches.Circle(
            (cx, cy), r, facecolor=GOLD, edgecolor=GOLD_DARK,
            linewidth=2.5, alpha=alpha, zorder=2,
        )
        ax.add_patch(circle)
        inner = patches.Circle(
            (cx, cy), r * 0.8, facecolor="none", edgecolor=GOLD_DARK,
        ...
        ax.text(
            ...
            fontsize=r * 55, fontweight="bold", color=EDGE,
            alpha=alpha, zorder=4, family="sans-serif",
        )


    def draw_urn(ax, x0, y0, n_white, n_black, cols=4, ball_r=0.18,
                removed_ball=None, removed_pos=None):
        ...
        urn = patches.FancyBboxPatch(
            ...
            edgecolor="#6b4a2b", facecolor="#faf6ee",
        ...
        for i, color in enumerate(colors):
            ...
            ball = patches.Circle(
                (cx, cy), ball_r, facecolor=face, edgecolor=EDGE,
    ...
    ax = axes[1, 1]
    draw_urn(
        ax, 0.6, 0.6, n_white=6, n_black=5, cols=4, ball_r=0.2,
        removed_ball="black", removed_pos=(2.55, 2.25),
    )
...
def gora_shapes():
    ...
    population = FancyBboxPatch(
        ...
        facecolor=POP_FILL, edgecolor=POP_EDGE, linewidth=2.2, zorder=1,
    )
    ax.add_patch(population)

    ax.text(5, 6.85, "All genes in genome / background set (N)",
    ...
    circle_go = Circle(c1, r1, facecolor=GO_COLOR, edgecolor="white",
                        linewidth=2.5, alpha=0.85, zorder=2)
    circle_de = Circle(c2, r2, facecolor=DE_COLOR, edgecolor="white",
    ...
    ax.text((c1[0] + c2[0]) / 2 + 0.15, c1[1], "k",
    ...
    def label_with_leader(ax, point, text_xy, text, ha="center", color=TEXT_DARK):
        ax.annotate(
            text, xy=point, xytext=text_xy,
            ha=ha, va="center", fontsize=12, fontweight="bold", color=color,
            arrowprops=dict(arrowstyle="-", color=LEADER, lw=1.4,
                            shrinkA=2, shrinkB=6),
            zorder=6,
        )

    label_with_leader(
        ax, point=(c1[0] - 1.1, c1[1] + 0.9), text_xy=(1.3, 6.0),
        text="GO-term annotated\ngenes (K)", ha="left",
    )
    label_with_leader(
        ax, point=(c2[0] + 1.05, c2[1] + 0.7), text_xy=(8.7, 6.0),
        text="Differentially\nexpressed genes (n)", ha="right",
    )
