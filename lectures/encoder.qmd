...
- Positional encodings use an index $i\in {0,1,\ldots,\frac{d}{2}-1}$ that indexes **pairs** of dimensions:
    - $2i$ (sine)
    - $2i+1$ (cosine)
- Both the sine and the cosine dimensions for $i$ use the same frequency $\omega_{i} = (\frac{1}{10000})^{2i/d}$
- For sine (output dimension slot $2i$):
$$
\mathrm{PE}(pos,2i) = \sin\left(\dfrac{pos}{10000^{\frac{2i}{d}}}\right)
$$

- For cosine (output dimension slot $2i+1$):

$$
\mathrm{PE}(pos,2i+1) = \cos\left(\dfrac{pos}{10000^{\frac{2i}{d}}}\right)
$$
...
:::{.small-text}
- example: **first word** ($k=0$)
  - $k=0, i=0$, <span style="color:blue">$2i=0$</span>; $\mathrm{PE}(k,2i) = \sin\left(\frac{k}{100^{\frac{2i}{d}}}\right) = \sin\left(\frac{0}{100^{\frac{0}{4}}}\right) = \sin(0)=0$
  - $k=0, i=0$, <span style="color:blue">$2i + 1=1$</span>; $\mathrm{PE}(k,2i+1) = \cos\left(\frac{k}{100^{\frac{2i}{d}}}\right) = \cos\left(\frac{0}{100^{\frac{0}{4}}}\right) = \cos(0)= 1$
  - $k=0, i=1$, <span style="color:blue">$2i=2$</span>; $\mathrm{PE}(k,2i) = \sin\left(\frac{k}{100^{\frac{2i}{d}}}\right) = \sin\left(\frac{0}{100^{\frac{2}{4}}}\right) =\sin(0)=0$
  - $k=0, i=1$, <span style="color:blue">$2i+1=3$</span>; $\mathrm{PE}(k,2i+1) = \cos\left(\frac{k}{100^{\frac{2i}{d}}}\right) = \cos\left(\frac{0}{100^{\frac{2}{4}}}\right) = \cos(0)= 1$
- The slot (e.g., $2i$ or $2i+1$) determines the index in the positional encoding vector
- $k$ is the index in the original sequence
...
- Continuing our example with the second word "am"
...
:::{.small-text}
- example: **second word** ($k=1$)
  - $k=1, i=0$, <span style="color:blue">$2i=0$</span>; $\mathrm{PE}(k,2i) = \sin\left(\frac{k}{100^{\frac{2i}{d}}}\right) = \sin\left(\frac{1}{100^{\frac{0}{4}}}\right) = \sin(1)=0.8415$
  - $k=1, i=0$, <span style="color:blue">$2i+1=1$</span>;; $\mathrm{PE}(k,2i+1) = \cos\left(\frac{k}{100^{\frac{2i}{d}}}\right) = \cos\left(\frac{1}{100^{\frac{0}{4}}}\right) = \cos(1)= 0.5403$
  - $k=1, i=1$, <span style="color:blue">$2i=2$</span>;; $\mathrm{PE}(k,2i) = \sin\left(\frac{k}{100^{\frac{2i}{d}}}\right) = \sin\left(\frac{1}{100^{\frac{2}{4}}}\right) =\sin(1/10)=0.0998$
  - $k=1, i=1$, <span style="color:blue">$2i+1=3$</span>;; $\mathrm{PE}(k,2i+1) = \cos\left(\frac{k}{100^{\frac{2i}{d}}}\right) = \cos\left(\frac{1}{100^{\frac{2}{4}}}\right) = \cos(1/10)= 0.9950$
:::


## Understanding the PE Matrix (3)

The denominator of the formula contains the "magic number" 100:
$$
\mathrm{PE}(k,2i) = \sin\left(\frac{k}{100^{\frac{2i}{d}}}\right) 
$$

- In the worked example above we used $n=100$ for clarity. The original "Attention Is All You Need" paper^[Vaswani et al. (2017) 31st Conference on Neural Information Processing Systems (NIPS)] uses $n=10000$.
- This base creates a geometric progression of "frequencies" across dimension pairs.
- At $i=0$ (smallest wavelength): the exponent $2i/d = 0$, so the denominator is $n^0 = 1$ regardless of $n$ — the wave oscillates fastest.
- At $i = d/2$ (largest wavelength): the exponent $2i/d = 1$, so the denominator is $n^1 = n$ — with $n=100$ that's $100$; with the paper's $n=10000$ that's $10000$. The wave oscillates slowest.
...
# Section 3: Multi-head attention {#multihead-attention}
...
::: {.callout-note icon=false, title="Add"}
