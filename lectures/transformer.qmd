...
- [Layer Normalization (2)](#layernorm2)
...
![](./img/pe-diagram.png){width=80% fig-align="center"}

- The positional encodings are computed once and reused for every sentence (training and test/inference)
...
- The transformer uses a FFN layer identical to the layers used in deep learning
:::

* the FFN transforms that information for each single word, adds necessary nonlinearity, stores factual knowledge, and holds most of the model's parameters
* The FFN enhances performance of the transformer by  introducing non-linear transformations at each token position.
* It consists of two linear transformations and a non-linear activation function (typically ReLU or GELU).
...
# Section 6: Layer Normalization {#layernorm2}
...
- The First Add & Norm stabilized the Attention layer.
- This Second Add & Norm stabilizes the FFN layer.
- (This ordering reflects the original Post-LN design;^[Vaswani et al. (2017) 31st Conference on Neural Information Processing Systems (NIPS)] Pre-LN architectures norm before each sublayer instead.)


$$\text{Layer Output} = \text{LayerNorm} \left( 
 \underbrace{X_{\text{post-attn}}}_{\text{Base}} + \underbrace{\text{FFN}(X_{\text{post-attn}})}_{\text{FFN output}}   
\right)
$$
...
- **The "Add" (Residual)**: ensures that even if the FFN didn't learn anything useful for a particular token (e.g. a common word like "the" or "a"), the original context from the attention layer is preserved.
- **The "Norm" (LayerNorm)**: rescales the $d$ features so their magnitude doesn't grow progressively larger from layer to layer (e.g. $d=768$ across $L=12$ layers for BERT-Base).
- The output of this final Norm has exactly the same shape as the input at the very start of the layer: $n \times d$.
...
This lecture introduced the Encoder architecture. 
- In the next lecture, we will introduce the decoder architecture and discuss how these models are trained.
:::
