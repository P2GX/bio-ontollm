...
- With MCMC, our samples are on a Markov chain
...
- After burn-in, the chain's samples $x_i$ are (approximately) draws from $p(x)$ 
...
p(x)\,T(y\mid x) = p(y)\,T(x\mid y)
$$
- Assume we are in the stationary distribution sunny: $\pi_1 =\frac{5}{12}$ and cloudy: $\pi_2 =\frac{7}{12}$
$$
\underbrace{p(S)\,T(R\mid S)}_{\frac{5}{12}\times 0.7 \,=\, \frac{7}{24}} \;=\; \underbrace{p(R)\,T(S\mid R)}_{\frac{7}{12}\times 0.5 \,=\, \frac{7}{24}} \quad\checkmark
$$
- Thus, $p$ is a stationary distribution — we can also check directly:
$$
\sum_x p(x)\,T(S\mid x) = \underbrace{\tfrac{5}{12}\times 0.3}_{\text{stay Sunny}} + \underbrace{\tfrac{7}{12}\times 0.5}_{\text{Rainy}\to\text{Sunny}} = \tfrac{15}{120} +  \tfrac{35}{120}= \tfrac{5}{12} = p(S) \quad\checkmark
$$
...
- The Metropolis-Hastings (MH) algorithm is a general method for constructing a transition kernel $T$ that samples from an arbitrary target $p$, using a **proposal** and an **acceptance** step
- $q(y\mid x)$ is the proposal — a candidate generator, saying what state *might* come next
...
$$
- It's $T$ — not $q$ — that needs to satisfy detailed balance
- $a(x,y)$'s $\min(1,\cdot)$ form is *designed* so that $T = q\cdot a$ satisfies detailed balancefor **any** valid proposal $q$

## MH — Proof (continued)

- Let $a, b$ be two states with $p(b) < p(a)$. Then $a(a,b) = \dfrac{p(b)}{p(a)} < 1$ — the probability of accepting a move from $a$ to $b$ is less than one
- What about the reverse move, $b \to a$? Since $p(a) > p(b)$, moving to $a$ only *increases* probability, so we always accept:
$$
a(b,a) = \min\!\left(1,\ \frac{p(a)}{p(b)}\right) = 1 \qquad \text{(since } \tfrac{p(a)}{p(b)} > 1\text{)}
$$
- Now check detailed balance, $p(a)T(b\mid a) \overset{?}{=} p(b)T(a\mid b)$, using $T(y\mid x) = q(y\mid x)\,a(x,y)$:
$$
p(a)\,T(b\mid a) = p(a)\,q(b\mid a)\,a(a,b) = p(a)\,q(b\mid a)\cdot\frac{p(b)}{p(a)} = q(b\mid a)\,p(b)
$$
$$
p(b)\,T(a\mid b) = p(b)\,q(a\mid b)\,a(b,a) = p(b)\,q(a\mid b)\cdot 1 = q(a\mid b)\,p(b)
$$

## MH — Proof (continued)

- These are equal **provided $q$ is symmetric**, i.e. $q(b\mid a) = q(a\mid b)$:
$$
p(a)\,T(b\mid a) = q(b\mid a)\,p(b) = q(a\mid b)\,p(b) = p(b)\,T(a\mid b) \qquad\checkmark
$$
- So for a symmetric proposal, this simplified acceptance rule — accept automatically if $p(b)\geq p(a)$, otherwise accept with probability $p(b)/p(a)$ — guarantees detailed balance
- This is **Metropolis**, i.e., the proposal distribution is symmetric
- In the general case (**Metropolis-Hastings**),  if $q$ isn't symmetric, the $q(b\mid a)$ and $q(a\mid b)$ terms above don't cancel


## MH — Proof (continued): the general case

- Drop the assumption $p(b) < p(a)$ — the general proof works for **any** two states, no case split needed
- Write out $a(a,b)$ and $a(b,a)$ using the full MH formula:
$$
a(a,b) = \min\!\left(1,\ \frac{p(b)\,q(a\mid b)}{p(a)\,q(b\mid a)}\right), \qquad
a(b,a) = \min\!\left(1,\ \frac{p(a)\,q(b\mid a)}{p(b)\,q(a\mid b)}\right)
$$
- Now compute $p(a)\,T(b\mid a) = p(a)\,q(b\mid a)\,a(a,b)$:
$$
p(a)\,q(b\mid a)\,a(a,b) = p(a)\,q(b\mid a)\cdot\min\!\left(1,\ \frac{p(b)\,q(a\mid b)}{p(a)\,q(b\mid a)}\right)
$$
- Pull $p(a)\,q(b\mid a)$ **inside** the $\min(\cdot)$ (valid since it's a positive constant):
$$
= \min\!\Big(p(a)\,q(b\mid a),\ \ p(b)\,q(a\mid b)\Big)
$$
- By the same steps, $p(b)\,T(a\mid b) = p(b)\,q(a\mid b)\,a(b,a)$ simplifies to:
$$
p(b)\,q(a\mid b)\,a(b,a) = \min\!\Big(p(b)\,q(a\mid b),\ \ p(a)\,q(b\mid a)\Big)
$$
...
- Thus
$$
p(a)\,T(b\mid a) = \min\big(p(a)q(b\mid a),\ p(b)q(a\mid b)\big) = \min\big(p(b)q(a\mid b),\ p(a)q(b\mid a)\big) = p(b)\,T(a\mid b) \qquad\checkmark
$$
- **Detailed balance holds exactly**, for *any* proposal $q$ — symmetric or not, and regardless of which of $p(a), p(b)$ is larger
- Recall from earlier: **detailed balance $\Rightarrow$ stationarity** — summing both sides over $a$ recovers $\sum_a p(a)\,T(b\mid a) = p(b)$
...
- We accept moves according to the Hastings ratio, $h=\min\left(1, \frac{p(a)q(b\mid a)}{p(b)q(a\mid b)}\right)$
- Since we are using an uniform transition kernel, $q(b\mid a) = q(a\mid b)$ and thus $h=\min\left(1, \frac{p(a)}{p(b)}\right)$
...
&= \log \frac{\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(a-\mu)^2}{2\sigma^2}}}{\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(b-\mu)^2}{2\sigma^2}}}\\
&= \log \frac{e^{-\frac{(a-\mu)^2}{2\sigma^2}}}{e^{-\frac{(b-\mu)^2}{2\sigma^2}}}\\
&= \frac{1}{2\sigma^2}\left[ (a-\mu)^2 - (b-\mu)^2\right]
\end{align}
$$

- We sample a random number $r$ uniformly from the interval $[0,1]$. If $ h < r$, then we accept the proposal and advance the chain from $a$ to $b$. If not, we remain at $a$.


## MCMC: Run with start position $x_0=0$, $d=1$
...
- Acceptance rate of about 33% is considered optimal - maybe the proposal distribution of 1 is too small and we are not exploring the space enough?
:::
