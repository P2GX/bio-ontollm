...
## Three Task Types, One Encoder

- All three tasks below reuse the *same* pretrained BERT encoder — only the head on top, and which output positions it reads, differ.

| Task type | Reads from | Head |
|---|---|---|
| Sentence classification | `[CLS]` only | 1 linear layer → softmax over classes |
| Token classification | every token | 1 linear layer (shared) → softmax per token |
| Question answering (span extraction) | every token, over the context | 2 linear layers (start / end logits) |


## Task 1: Sentence Classification

- Example: sentiment classification — "The movie was great" → {positive, negative}
- Input: `[CLS] The movie was great [SEP]`
- Only the `[CLS]` position's final hidden state $\mathbf{h}_{\texttt{[CLS]}} \in \mathbb{R}^{d}$ is used:

$$
P(\text{class}) = \text{softmax}(\mathbf{h}_{\texttt{[CLS]}} W_{\text{cls}} + b_{\text{cls}})
$$

- $W_{\text{cls}} \in \mathbb{R}^{d \times C}$, where $C$ is the number of classes — this is the *only* new set of learnable parameters introduced by fine-tuning.
- All of BERT's existing weights (attention, FFN, LayerNorm, ...) can either be fine-tuned (updated further) or frozen (left as-is) — fine-tuning the whole model usually performs better, but freezing is cheaper.


## Worked Example: Sentence Classification
...
used = ["✓ (used for classification)", "", "", "", "", ""]

df = pd.DataFrame({"Token": tokens, "Used by classification head?": used})
df
```
:::

- Only the first row (`[CLS]`) feeds the classification head — the other five tokens' final representations exist and were computed, but this task simply doesn't use them.
- This is different from the next task, where *every* token's output matters.


## Task 2: Token Classification

- Example: Named Entity Recognition (NER) — label each token as `PERSON`, `LOCATION`, `ORG`, or `O` (none)
- Input: `[CLS] Peter works at Anthropic [SEP]`
- Now **every** token position's final hidden state $\mathbf{h}_i \in \mathbb{R}^d$ gets its own prediction, using the *same* shared linear layer applied independently at each position:
...
## Worked Example: Token Classification
...
tokens = ["[CLS]", "Peter", "works", "at", "Anthropic", "[SEP]"]
labels = ["—", "B-PERSON", "O", "O", "B-ORG", "—"]

df = pd.DataFrame({"Token": tokens, "Predicted label": labels})
df
```
:::

- Every real token gets its own independent prediction from the shared head — "Peter" and "Anthropic" are correctly tagged as the start of a person/organization span, while "works" and "at" are tagged `O` (outside any entity).


## Task 3: Question Answering (Span Extraction)

- Example: given a context passage and a question, extract the exact substring of the context that answers it.
- Input: `[CLS] Question [SEP] Context [SEP]`
- Two separate linear heads are applied to every token in the **context** portion, producing a start-logit and an end-logit per token:

$$
\text{start\_logit}_i = \mathbf{h}_i \cdot \mathbf{w}_{\text{start}}, \qquad \text{end\_logit}_i = \mathbf{h}_i \cdot \mathbf{w}_{\text{end}}
$$

- $\mathbf{w}_{\text{start}}, \mathbf{w}_{\text{end}} \in \mathbb{R}^d$ are learned vectors — softmax over each set of logits gives a probability distribution over "where does the answer start" and "where does it end."
- The predicted answer span is the substring between $\arg\max$ of the start distribution and $\arg\max$ of the end distribution.


## Worked Example: Question Answering

- Question: `[CLS] Where does Peter work? [SEP]`
- Context: `Peter works at Anthropic in San Francisco [SEP]`
...
tokens = ["Peter", "works", "at", "Anthropic", "in", "San", "Francisco"]
start_p = [0.02, 0.01, 0.03, 0.85, 0.04, 0.03, 0.02]
end_p   = [0.01, 0.02, 0.02, 0.80, 0.05, 0.03, 0.07]

df = pd.DataFrame({"Token": tokens, "P(start)": start_p, "P(end)": end_p})
df
```
:::

- Highest start probability and highest end probability both land on "Anthropic" — so the extracted answer span is just "Anthropic," a single token.
- For a multi-token answer (e.g. "San Francisco"), the start distribution would peak at "San" and the end distribution would peak at "Francisco," with everything between included in the span.
