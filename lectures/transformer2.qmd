...
- Adding $-\infty$ to a score, then applying softmax, sends that entry's contribution to exactly $0$ — the future token is mathematically removed from the weighted sum, not just "discouraged."
...
- This lower-triangular pattern (0 on and below the diagonal, $-\infty$ above) is why causal masking is often just called "triangular masking."


## Worked Example: From Raw Scores to Masked Attention Weights

Suppose the raw attention scores $\frac{QK^\top}{\sqrt{d_k}}$ for our sentence are:
...
Adding the mask $M$ and applying softmax row-by-row:
...
# Build the causal mask: 0 on/below diagonal, -inf above
M = np.triu(np.full((n, n), -np.inf), k=1)
np.fill_diagonal(M, 0)
M = np.tril(np.zeros((n, n))) + np.triu(M, k=1)

masked_scores = S + M

# Row-wise softmax (ignoring -inf entries)
weights = np.zeros_like(S)
for i in range(n):
    row = masked_scores[i]
    finite = np.isfinite(row)
    exp_row = np.zeros_like(row)
    exp_row[finite] = np.exp(row[finite] - row[finite].max())
    weights[i] = exp_row / exp_row.sum()

df = pd.DataFrame(weights, index=tokens, columns=tokens).round(3)
df
```
:::
...
- Notice the upper triangle is exactly $0$ in every row — this is the visual signature of causal masking, and it directly mirrors the $n\times n$ score matrix from Part 1's "Visualizing the $n\times n$ Matrix" slide, just with the upper triangle zeroed out.


## Connecting Back to Part 1

- The scaling factor $\frac{1}{\sqrt{d_k}}$, the softmax normalization, the variance argument for why we scale — **none of that changes**. Everything from Part 1's derivation still applies row-by-row.
- The *only* new ingredient is the additive mask $M$, applied **before** softmax.
- This is a general pattern worth remembering: masking-before-softmax is how transformers implement *any* kind of "this token may not attend to that token" constraint — causal masking is the decoder's specific use of it, but the same mechanism (additive $-\infty$ mask) shows up elsewhere too (e.g. padding masks for variable-length sequences).
