...
import matplotlib.pyplot as plt
from reinforce_plots import training_stages
fig = training_stages(include=["Stage 2", "Stage 3"])
plt.show()
```

- Instruction fine tuning teaches the model to answer questions (to be a chatbot)
- Preference fine tuning (Reinforcement learning) teaches the model to respond in ways that correspond to human preferences


## Today

- Today we will focus on instruction fine tuning and reinforcement learning with human feedback 
- There is more than one way to do it: There is an enormous variety of posttraining approaches that we will not cover today. This is where the magic happens in modern LLMs, but it would require an additional course to do it justice
- You will learning some basic ideas of posttraining that will put you in a good position to understand more sophisticated approaches

## IFT (Instruction Fine-Tuning) and SFT (Supervised Fine-Tuning) 

::: {.mybox}
** IFT vs SFT**:

- These terms are nearly synonyms
:::
...
- **IFT**: A specific kind of SFT where the training data is specifically instruction/response pairs (e.g. "Summarize this text: ..." → summary; "Write a poem about X" → poem). 

The goal of IFT is to get the model to behave like a chatbot, i.e., to follow natural lanugage instructions and conduct a conversation, rather than just continuing the input text
...
**Response:** What is the most delicious Spanish appetizer? What is the most delicious Mexican appetizer? What is the most delicious Portuguese appetizer? These are the kinds of questions readers ask us all the time here on the blog, so we decided to put together a definitive ranking...
:::
...
<thead>
<tr><td>Item</td><td>Pretraining</td><td>IFT</td></tr>
</thead>
<tbody>
...
* **Learning Rate**: Lower the learning rate during instruction fine-tuning (IFT) to prevent overfitting on the narrower data distribution.
* **Loss Masking**: Apply cross-entropy (CE) loss exclusively on the assistant tokens rather than the prompt tokens.
...
sys.path.append('utils') 
import matplotlib.pyplot as plt
from reinforce_plots import pipeline
fig = pipeline()
plt.show()
```
...
- Reinforcement learning for LLMs is a very active area of research and there are many different approaches and algorithms
- In this lecture, we will focus on the approach presented in the InstructGPT paper
- This approach is no longer used in modern models, but it is a good place to start to learn about LLM-RL
- At the end of the talk, we will present a selected list of new developments
:::
...
-- todo, summarize learn next token
...
- Why exactly are pretrained models not ready to chat with users?

## Pretraining 
::: {.mybox}
**Base Model**:

- Pretrained LLMs are trained to do next-token prediction or text continuation based on web-scale statistical association.
:::
...
- a language model trained only to predict the most likely continuations might respond:

> Translate the following English sentence to Spanish: "The weather is nice today!" Also, I want to know how to order coffee in Spanish?

- Whereas actually, we want a response such as

> Translate the following English sentence to Spanish: "The weather is nice today!" "El tiempo está precioso hoy."
...
::: {.mybox}
- Right after pretraining, the LLM is essentially trained to autocomplete
- This is not necessarily what humans want.
- To implement a chatbot, the LLM needs to respond in a question/answer format
:::

- Start from a pretraining language model
- Create a collection of desired model behavior (e.g., question answering)
- Train with standard supervised learning on prompt-response pairs
- Following SFT,^[SFT is also known as Instruction fune tuning (IFT)] the model can answer questions 
- SFT can be used to adapt the base LLM to many other domains
