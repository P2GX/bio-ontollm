...
- Since it is convenient to minimize the function, we multiply by -1, and take this to be be cross-entropy loss
$$
...
$$
- Remember that $\hat{y}$ (the prediction) is equal to $\sigma(x)$, and thus $\sigma(x)(1-\sigma(x)) = \hat{y}(1-\hat{y})$


## Derivation and chain rule


- To derive the above expression (and to understand backprop in the next lecture!), it is essential to be familiar with the chain rule
...
## Chain rule $\mathcal{L}$ w.r.t. $\hat{y}$($w_j$)

- $\mathcal{L} = -(y \ln \hat{y} + (1-y) \ln(1-\hat{y}))$, recalling that $\frac{d \log x}{dx} = \frac{1}{x}$


$$
$$\begin{eqnarray*}
\frac{\partial \mathcal{L}}{\partial \hat{y}} &=& -\frac{\partial }{\partial \hat{y}} [y \ln \hat{y} + (1-y) \ln(1-\hat{y})] \\
&=& -\frac{y}{ \hat{y}}  +   (1-y)  \frac{-1}{1-\hat{y}} = -\left( \frac{y}{\hat{y}} - \frac{1-y}{1-\hat{y}} \right) \\
&=&  \frac{-y}{\hat{y}} + \frac{1-y}{1-\hat{y}}  = \frac{-y}{\hat{y}}\cdot \frac{1-\hat{y}}{1-\hat{y}}  + \frac{1-y}{1-\hat{y}}  \cdot \frac{\hat{y}}{\hat{y}} \\
...
## Chain rule:  Sigmoid $\hat{y}$ w.r.t. the Logit $z$O
...
Intuitively,
The Error ($\hat{y} - y$): How wrong were we?The Input ($x_j$): How much did this specific feature contribute to that wrong prediction?