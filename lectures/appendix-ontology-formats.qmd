...
subtitle: "Appendix: Ontology formats — OBO, RDF, RDFS, and OWL"
...
This deck collects the material on ontology **file formats** and the Semantic
Web stack that was previously part of Lecture 1. It is background reading for
students who want to know how ontologies are serialized and exchanged; the
later lectures do not build on it.
:::

* **OBO** --- the flat-file format used by the Gene Ontology and the OBO Foundry
* **RDF** --- the Semantic Web data model of subject-predicate-object triples
* **RDFS** --- a schema language on top of RDF that supports inference
* **OWL** --- the Web Ontology Language and its logical class constructors

::: {.callout-tip appearance="simple"}
Lecture 1 keeps only the *is a* semantics from this material, because that is
what the Gene Ontology, MGSA, and the semantic similarity algorithms in the
later lectures actually rely on.
:::
---
...
* __transitive__: If A part_of B and B part_of C, then A part_of C.
...
::: {.callout-note appearance="minimal" icon=false style="background-color: #f4f6f9; border-left: 4px solid #3a6073; padding: 15px;"}
...
<rdf:RDF xmlns="http://purl.uniprot.org/core/" 
  ...
  <rdf:Description rdf:about="http://purl.uniprot.org/enzyme/1.14.11.2">
...
- Enzyme Commission 	[EC 1.14.11.2](https://www.genome.jp/dbget-bin/www_bget?ec:1.14.11.2)
...
5. 
6.   <rdf:Description 
7.        rdf:about="http://purl.uniprot.org/enzyme/1.14.11.2">
8.     <rdf:type rdf:resource="http://purl.uniprot.org/core/Enzyme"/>
...
* Thus, if we know that Mickey is an instance of \textit{mouse}
we can infer that Mickey is an instance of the class \textit{rodent}!
...
X rdfs:subClassOf Z .
```
* Thus, we can infer that `mouse` is a subclass of the class `rodent`! 
...
```text
x:Mouse rdfs:subClassOf x:Mammal .
```
...
```text
@prefix r: http://www.regulation.org .
...
::: {.callout-warning title="Limits of RDFS"}
...
Then, if either of the two following statements are asserted
...
![](./img/Winniethepooh.png){fig-align="center" width="20%"}
...
![](./img/beatles.png){fig-align="center" width="25%"}
...
**Cardinality Constraints**. A value restriction $V$ on some property 
$P$ has the following general form:
...
* **Formal Logic:** Recall that the formula $(\forall x) P(x)$ essentially means "for all $x$, if $x$ exists in this context, then it must satisfy $P$."[^1]

[^1]: According to this logic it is true that "All Martians are green" (because there are no Martians!)
