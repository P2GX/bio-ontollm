...
subtitle: "Session #6: Neural networks"
...
- Fully connected layer. In a Feed-Forward Network (FFN), a [Fully Connected layer]{style="color: green; font-weight: bold;"} — also 
known as a __Dense layer__ or __Linear layer__—is a layer where every input neuron 
is connected to every output neuron: $$z = \sum_{i=1}^{4} (w_i x_i) + b$$
...
* Activation functions decide whether a neuron should be "fired" (activated) by calculating 
the weighted sum and adding bias.
...
draw_nn_forward_pass(figsize=(8,5))
plt.plot()
```
...
draw_nn_backpropagation(figsize=(8,5))
plt.plot()
```
...
- $w_{j,i}$: the __weights__. They are __learnable__ parameters
- $z_j$: the __weighted sums__ or the __pre-activations__.
- $a_j$: the __activations__ and these are passed on as the input to the next layer
:::
...
A single ANN [layer]{style="color: green; font-weight: bold;"} implements a three-step cycle:

::: {.columns}

::: {.column width="45%"}
### 1. Weighted Sum ($\mathbf{z}$)
The input vector $\mathbf{x}$ (or activations from the previous layer $\mathbf{a}_{l-1}$) is multiplied by the weight matrix $W$ and the bias vector $\mathbf{b}$ is added.
$$\mathbf{z} = W \mathbf{x} + \mathbf{b}$$

### 2. Activation ($\mathbf{a}$)
...
::: {.column width="55%"}
### 3. Propagation
The resulting activation vector $\mathbf{a}$ is output and becomes the incoming input vector for the subsequent layer.

::: {.callout-tip icon=false title="Summary/Nomenclature"}
1. **Input Layer**: Receives raw features $\mathbf{x}$ (e.g., genomic features).
2. **Hidden Layer(s)**: 
    - Compute weighted sums of inputs.
    - Apply activation functions to introduce non-linearity.
3. **Output Layer**: Produces the final prediction $\hat{y}$ (e.g., disease risk probability).
4. **Foeward pass**: Perform this three-step forward pass sequentially through all layers to compute the final output.
:::
:::
...
draw_forward_block_diagram(fig_size = (7, 2.5))
plt.plot()
```
...
- We do this via [gradient descent]{style="color: green; font-weight: bold;"}. 
...
- We will use a squared-error loss function (where $y$ is the label, i.e.,  zero or one) and $a_3$ is the output of the ANN:
$$
...
$$
\begin{eqnarray*}
\dfrac{d\sigma(x)}{dx} =\dfrac{d}{dx} \dfrac{1}{1+e^{-x}} &=&\sigma(x) \cdot \left( 1 -\sigma(x) \right)
\end{eqnarray*}
$$

- Refer to the [previous lecture]{style="color: green; font-weight: bold;"} for details of the derivation.
- The forward pass calculates $\sigma(x)$. Using this derivation, the backward pass can calculate the partial derivate __easily__ using the value of $\sigma(x)$.
...
$$
\mathbf{f}(x) = \begin{bmatrix} 4x^2-7x +42\\ x^9 -21\end{bmatrix}
$$
then
$$
\dfrac{\partial \mathbf{f}(x)}{\partial x}
=
...
$$
\dfrac{\partial f(\mathbf{x})}{\partial \mathbf{x}} = 
...
- Given $\mathbf{y} = \sigma(\mathbf{x})$, the derivative $\frac{\partial \mathbf{y}}{\partial \mathbf{x}}$ is a **Jacobian Matrix**. 
- Since $y_i$ only depends on $x_i$ (and no other $x_j$), the matrix is diagonal:
...
* **Output vector:** $\mathbf{y} = [\sigma(x_1), \sigma(x_2), \dots, \sigma(x_n)]^T$

Therefore, because the input and output dimensions match ($n = m$), this is a square (and diagonal/symmetrical) matrix.
:::
...
- See the [previous lecture]{style="color: green; font-weight: bold;"} for an introduction to gradient descent in the setting of logistic regression.
